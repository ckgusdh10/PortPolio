...
void UIShader::CreatePSO(ID3D12Device *pd3dDevice, UINT nRenderTargets, UINT nIndex)
{
	...
	d3dPipelineStateDesc.PS = CreatePixelShader(&pd3dPixelShaderBlob, nIndex);
...
D3D12_SHADER_BYTECODE UIShader::CreateVertexShader(ID3DBlob **ppd3dShaderBlob, UINT nIndex)
{
	...
	if (nIndex == WARRIORSKILL)
		return(CompileShaderFromFile(L"UI.hlsl", "WARRIORSKILL_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == FAIRYSKILL)
		return(CompileShaderFromFile(L"UI.hlsl", "WARRIORSKILL_VS", "vs_5_1", ppd3dShaderBlob));
	if (nIndex == SANYANGSKILL)
		return(CompileShaderFromFile(L"UI.hlsl", "WARRIORSKILL_VS", "vs_5_1", ppd3dShaderBlob));
}
...
D3D12_SHADER_BYTECODE UIShader::CreatePixelShader(ID3DBlob **ppd3dShaderBlob, UINT nIndex)
{
	if (nIndex == WARRIORSKILL)
		return(CompileShaderFromFile(L"UI.hlsl", "WARRIORSKILLPS", "ps_5_1", ppd3dShaderBlob));
	if (nIndex == FAIRYSKILL)
		return(CompileShaderFromFile(L"UI.hlsl", "SANYANGSKILLPS", "ps_5_1", ppd3dShaderBlob));
	if (nIndex == SANYANGSKILL)
		return(CompileShaderFromFile(L"UI.hlsl", "FAIRYSKILLPS", "ps_5_1", ppd3dShaderBlob));
...
{
	//						개수
	m_pTexture = new CTexture(11, RESOURCE_TEXTURE2D, 0);
	...
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/warriorSkill.dds", 8);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/fairyskill.dds", 9);
	m_pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Assets/UI/sanskill.dds", 10);
...
void UIShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	if (m_pPlayer)
	{
		...
		m_ui.skilltime = m_pPlayer->warriorskilltime;
		m_ui.sanyangskilltime = m_pPlayer->sanyangskilltime;
		m_ui.fairyskilltime = m_pPlayer->fairyskilltime;
	}
	...
	for (int i = 0; i < m_nPipelineStates; i++)
	{
		...
		else if (i == 8 || i == 9 || i == 10)
		{
			if (m_pPlayer->GetCurMeshNum() == 0)
			{
				...
				pd3dCommandList->SetGraphicsRootDescriptorTable(0, m_pTexture->GetArgumentInfos(8).m_d3dSrvGpuDescriptorHandle);

			}
			else if (m_pPlayer->GetCurMeshNum() == 1)
			{
				if (m_ppd3dPipelineStates)
					pd3dCommandList->SetPipelineState(m_ppd3dPipelineStates[9]);

				pd3dCommandList->SetGraphicsRootDescriptorTable(0, m_pTexture->GetArgumentInfos(9).m_d3dSrvGpuDescriptorHandle);

			}
			else if (m_pPlayer->GetCurMeshNum() == 2)
			{
				if (m_ppd3dPipelineStates)
					pd3dCommandList->SetPipelineState(m_ppd3dPipelineStates[10]);

				pd3dCommandList->SetGraphicsRootDescriptorTable(0, m_pTexture->GetArgumentInfos(10).m_d3dSrvGpuDescriptorHandle);
